...
\usepackage{fancyhdr}
\pagestyle{fancy}
\lhead{A 2-Norm Condition Number for B\'{e}zier Curve Intersection}
\rhead{Danny Hermes}
...
\\ \\
\noindent \emph{Keywords}: B\'{e}zier curve, Curve intersection,
...
Throughout, we'll refer to a \emph{B\'{e}zier curve} as a parametric
...
combination of the set of \emph{control points} \(\bm{b}_j \in \reals^2\).

A B\'{e}zier curve intersection is a root \(\left[\begin{array}{c}
\alpha \\ \beta \end{array}\right]\) of the function
\begin{equation}
F(s, t) = b_0(s) - b_1(t)
\end{equation}
where \(b_0(s)\) and \(b_1(t)\) are B\'{e}zier curves. Note that
...
\section{Conditioning of Generic Root-finding}
...
Thus we know the maximum occurs at two of the four corners of the
...
This low condition number is somewhat unexpected since when using
the resultant to eliminate each parameter, one of the two roots is
a double root:
\begin{align}
...
coefficients grow so large that their ratio approaches \(1\).
